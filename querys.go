...
const GET_HANDOUTS_WITH_USERS = `
		SELECT h.id, h.amount, h.date, h.created_at, h.updated_at,
...
const CREATE_HANDOUTS = "INSERT INTO handouts (date, amount, user_id) VALUES ($1, $2, $3);"

const DELETE_HANDOUTS = "DELETE FROM handouts WHERE id = $1"

const UPDATE_HANDOUT = `UPDATE handouts SET date = $1, amount = $2, user_id = $3 WHERE id = $4`
